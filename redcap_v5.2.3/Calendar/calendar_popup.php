		...
		if (isset($row['record']) && $row['record'] != "") {	
			...
			if ($row['event_id'] != "") 
			{
				...
				if (db_num_rows($q) > 0) 
				{
					...
					$q = db_query("select field_name, value from redcap_data where project_id = $project_id and record = '".prep($row['record'])."' and 
									  event_id = {$row['event_id']} and field_name in ('".implode("_complete','",$form_names)."_complete')");
					while ($row4 = db_fetch_array($q)) {
