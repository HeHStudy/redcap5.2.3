...
// Open pop-up dialog for viewing data cleaner of a field
function dataCleaner(field,event_id,record) {
	if (record == null) record = getParameterByName('id');
	var existing_record = $('form#form input[name="hidden_edit_flag"]').val();
	$('#data_cleaner').dialog('destroy');
	if (existing_record == 1) {
		$('#dc_var').html(field);
		$('#data_cleaner')
			.html('<p><img src="'+app_path_images+'progress_circle.gif" class="imgfix"> Loading...</p>')
			.dialog({ bgiframe: true, title: 'Data Query Log <span style="margin-left:10px;font-weight:normal;">(variable "'+field+'", record "'+record+'")</span>', modal: true, 
				width: 750, height: 600, zIndex: 3999, buttons: { 
				Close: function() { $(this).dialog('destroy'); } }
			});
	}
	$.post(app_path_webroot+"DataEntry/data_cleaner_popup.php?pid="+pid, { action: 'view', field_name: field, event_id: event_id, record: record, existing_record: existing_record }, function(data){
		if (existing_record == 1) {
			$('#data_cleaner').html(data);
			initWidgets();
			// Scroll to bottom
			// $('#data_cleaner').scrollTop($('#data_cleaner').height());
		} else {
			simpleDialog(data);
		}
	});
...
function dataCleanerSave(field,event_id,record) {
	// Set var
	if (record == null) record = getParameterByName('id');
	var dc_status = ($('#data_cleaner input[name="dc-status"]:checked').length) ? $('#data_cleaner input[name="dc-status"]:checked').val() : '';
	// Make ajax call
	$.post(app_path_webroot+"DataEntry/data_cleaner_popup.php?pid="+pid, { action: 'save', field_name: field, event_id: event_id, record: record,
		comment: $('#dc-comment').val(), 
		user_id_next_action: $('#dc-user_id_next_action').val(), 
		change_required_next_action: (($('#dc-change_required_next_action').length && $('#dc-change_required_next_action').prop('checked')) ? 1 : 0),
		change_performed: (($('#dc-change_performed').length && $('#dc-change_performed').prop('checked')) ? 1 : 0),
		response_requested_next_action: ($('#dc-response_requested_next_action').length ? $('#dc-response_requested_next_action').val() : ''),
		send_email: ($('#dc-send_email').prop('checked') ? 1 : 0), 
		high_priority: (($('#dc-high_priority').length && $('#dc-high_priority').prop('checked')) ? 1 : 0),
		status: dc_status,
		responded_to_request: ($('#dc-responded_to_request').length ? $('#dc-responded_to_request').val() : '0')
	}, function(data){
		if (data=='0') {
			alert(woops);
		} else {
			...
			$('#data_cleaner').html(json_data.content);
			initWidgets();
			setTimeout(function(){
				highlightTableRow('dc-clog_id_'+json_data.clog_id,3000);
				if ($('#dc-clog_id_a'+json_data.clog_id).length) highlightTableRow('dc-clog_id_a'+json_data.clog_id,3000);
				if ($('#dc-clog_id_b'+json_data.clog_id).length) highlightTableRow('dc-clog_id_b'+json_data.clog_id,3000);
				if ($('#dc-clog_id_c'+json_data.clog_id).length) highlightTableRow('dc-clog_id_c'+json_data.clog_id,3000);
			},100);
			// Change ballon icon for field
...
function initWidgets() {
	...
	if ($('.cal').length) $('.cal').datepicker({ 
		onSelect: function(){ $(this).focus(); try{ calculate();doBranching(); }catch(e){ } },
