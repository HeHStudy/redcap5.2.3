...
<?php if ($isIOS) { ?><link rel="apple-touch-icon-precomposed" href="<?php echo APP_PATH_IMAGES ?>apple-touch-icon.png"><?php } ?>
...
if ($date_deleted != "")
{
	...
	$deleteProjDialog = "{$lang['bottom_65']} <b>".
						format_ts_mysql(date('Y-m-d H:i:s', strtotime($date_deleted)+3600*24*PROJECT_DELETE_DAY_LAG)).
						"</b>{$lang['bottom_66']}";
	if ($super_user) {
		$deleteProjDialog .= "<br><br><b>{$lang['edit_project_77']}</b> {$lang['bottom_68']}";
	}
	$deleteProjDialog .= "<br><br>".RCView::a(array('href'=>APP_PATH_WEBROOT_FULL."?action=myprojects",'style'=>'text-decoration:underline;'), $lang['bottom_69']);
	...
	if ($super_user) {
		$deleteProjRestoreBtnText = '"'.cleanHtml2($lang['control_center_375']).'"';
		$deleteProjRestoreBtnJs = "function(){undelete_project($project_id)}";
	} else {
		$deleteProjRestoreBtnText = 'null';
		$deleteProjRestoreBtnJs = 'null';
	}
	?>
...
if ($status < 2 && !empty($user_rights))
{
	...
	if ($surveys_enabled && $user_rights['participants']) {
		$invitePart = "<div class='hang' style='position:relative;'><img src='".APP_PATH_IMAGES."send.png' class='imgfix'>&nbsp;&nbsp;<a href='".APP_PATH_WEBROOT."Surveys/invite_participants.php?pid=$project_id'>".$lang['app_22']."</a></div>";
		if ($status < 1) {		
	...
	if (!$longitudinal) {
		$addEditRecordPage = "DataEntry/index.php?pid=$project_id&page={$Proj->firstForm}";
	} else {
	...
	if ($status < 1) {		
		$dataEntry .=  "<div class='menuboxsub' style='position:relative;'>- ".$lang['bottom_64']."</div>";
	}
	...
	if (isset($_GET['child']) && $_GET['child'] != "") {
		...
		check_user_rights($this_app_name);
	}	
...
if ($status < 2 && $user_rights['data_export_tool'] != "0" && isset($mobile_project) && $mobile_project != "0") { 
	$appsMenu .= "<div class='hang'><img src='".APP_PATH_IMAGES."arrow_circle_double_135.png' class='imgfix'>&nbsp;&nbsp;<a href=\"" . APP_PATH_WEBROOT . "stuff.php?pid=$project_id\">{$lang['app_09']}</a></div>";
}
//Data Export Tool
if (isset($user_rights['data_export_tool']) && $user_rights['data_export_tool'] != "0") { 
	$appsMenu .= "<div class='hang'><img src='".APP_PATH_IMAGES."application_go.png' class='imgfix'>&nbsp;&nbsp;<a href=\"" . APP_PATH_WEBROOT . "DataExport/data_export_tool.php?pid=$project_id&view=simple_advanced\">{$lang['app_03']}</a></div>";
}
//Data Import Tool
...
if ($user_rights['user_rights']) { 
	$appsMenu .= "<div class='hang'><img src='".APP_PATH_IMAGES."user.png' class='imgfix'>&nbsp;&nbsp;<a href=\"" . APP_PATH_WEBROOT . "UserRights/index.php?pid=$project_id\">{$lang['app_05']}</a></div>";
}
...
if ($status < 2 && (($enable_plotting > 0 && $user_rights['graphical']) || ($enable_plotting < 1 && $super_user))) {
	$appsMenu .= "<div class='hang'><img src='".APP_PATH_IMAGES."chart_curve.png' class='imgfix'>&nbsp;&nbsp;<a href=\"" . APP_PATH_WEBROOT . "Graphical/index.php?pid=$project_id\">{$lang['app_13']}</a></div>";
}
// Data Quality
if ($status < 2 && ($user_rights['data_quality_design'] || $user_rights['data_quality_execute'])) {
	$appsMenu .= "<div class='hang'><img src='".APP_PATH_IMAGES."checklist.png' class='imgfix'>&nbsp;&nbsp;<a href=\"" . APP_PATH_WEBROOT . "DataQuality/index.php?pid=$project_id\">{$lang['app_20']}</a></div>";
}
// API
...
if ($status < 2 && $user_rights['reports']) {
	$appsMenu .= "<div class='hang'><img src='".APP_PATH_IMAGES."layout.png' class='imgfix'>&nbsp;&nbsp;<a href=\"" . APP_PATH_WEBROOT . "Reports/report_builder.php?pid=$project_id\">{$lang['app_14']}</a></div>";
}
$appsMenu .= "</div>";
